...
const MyPDFDocument = ({ data }) => {
    ...
    if (Array.isArray(data) && data.length > 0) {
        mostViewed = data.reduce((max, item) => {
            const totalViews = (item.view_count ?? 0);
            return totalViews > ((max.view_count ?? 0)) ? item : max;
        });

        mostDownloaded = data.reduce((max, item) => {
            const totalDownloads = (item.download_count ?? 0);
            return totalDownloads > ((max.download_count ?? 0)) ? item : max;
        });
    }
                        ...
                        <View style={[styles.summaryBox, { marginTop: 20 }]}>
                            ...
                            {mostViewed && (
                                <Text style={styles.text}>
                                    Most Viewed Note: <Text style={styles.highlight}>{mostViewed.title}</Text> — Total Views: {(mostViewed.view_count ?? 0)}
                                </Text>
                            )}
                            {mostDownloaded && (
                                <Text style={styles.text}>
                                    Most Downloaded Note: <Text style={styles.highlight}>{mostDownloaded.title}</Text> — Total Downloads: {(mostDownloaded.download_count ?? 0)}
                                </Text>
                            )}
                        </View>
                    </>
...
// PDF Download Button
const AdminNotesDownloadPDF = ({ data }) => {
...
export default AdminNotesDownloadPDF;
